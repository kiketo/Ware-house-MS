...
namespace WHMS.Services.Tests.ProductServiceTest
{
    ...
    public class CreateProduct_Should
    {
        ...
        [DataRow(-3)]
        [DataRow(-0.21)]
        public async Task Throw_Exception_If_Negative_Margin(double margin)
        {
            var dbName = ((nameof(CreateProduct_Should)) + (nameof(Throw_Exception_If_Negative_Margin)));
            var options = TestUtils.GetOptions(dbName);
            ...
            using (var assertContext = new ApplicationDbContext(options))
            {
                ...
                var ex = await Assert.ThrowsExceptionAsync<ArgumentException>(() => ( sut.CreateProductAsync("ProductName", null, null, 0, margin, null,user)));
